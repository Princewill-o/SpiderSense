...
const MenuItem = ({ icon, label, href, onClick, index, totalItems, isOpen }: MenuItemProps) => {
  ...
  const closeAnimation = async () => {
    ...
    for (let i = 0; i < scaleTransition.length; i++) {
      await animate.start({
        ...
        backgroundColor: `color-mix(in srgb, var(--foreground) ${Math.max(100 - i * 10, 40)}%, var(--background))`,
        transition: {
