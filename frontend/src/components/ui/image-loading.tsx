...
export default function ImageLoader({
  ...
  const seededRandom = useCallback((seed: number, min: number = 0, max: number = 1) => {
  ...
  useEffect(() => {
    ...
    for (let row = 0; row < rows; row++) {
      for (let col = 0; col < cols; col++) {
        cells.push({
          ...
          x: col * cellWithGap,
          y: row * cellWithGap,
          blinkDelay: seededRandom(index * 1.1, 0, blinkSpeed),
          fadeDelay: seededRandom(index * 2.3, 0, fadeOutDuration),
          initialOpacity: seededRandom(index * 3.7, 0.3, 1),
          color: null,
          index
        })
        index++
    ...
    setGridCells(cells)
  }, [mounted, dimensions.width, dimensions.height, gridSize, cellGap, blinkSpeed, fadeOutDuration, seededRandom])
