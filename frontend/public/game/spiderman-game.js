...
var RESOURCES_FOLDER_PATH = "/game/";
...
SpidermanGame.prototype.audioEnabled       = false;
...
SpidermanGame.prototype.load = function() {
	...
	document.addEventListener("keydown", function(e) {
		...
		if (!self.audioEnabled) {
			...
			// Try to play the current audio track
			if (!self.muted) {
	...
	if (audio && audio.play) {
		...
		if (playPromise !== undefined) {
			playPromise.catch(function(error) {
				...
				console.log('Audio autoplay prevented (user interaction required):', error.name);
			});
		}
		return playPromise;
	}
...
SpidermanGame.prototype.spawnBoss = function() {
	if (this.bossActive) return;
	
	this.bossActive = true;
	var bossTypes = ["VENOM", "GOBLIN", "DOCTOR_OCTOPUS", "MYSTERIO", "SANDMAN"];
	var bossType = bossTypes[Math.min(this.level - 1, bossTypes.length - 1)];
	
	// Find a roof to spawn boss on
	var roofs = this.scene.roofs;
	var spawnRoof = roofs[roofs.length - 1];
	
	var boss = new Boss(this, {
		x: spawnRoof.x + spawnRoof.width / 2,
		y: spawnRoof.y - 100,
		bossType: bossType,
		level: this.level
	});
...
SpidermanGame.prototype.drawBossHealthBar = function() {
	if (!this.currentBoss) return;
	
	var boss = this.currentBoss;
	var barWidth = this.canvas.width - 40;
	var barHeight = 20;
	var x = 20;
	var y = 35;
	...
	var gradient = this.ctx.createLinearGradient(x, y, x + healthWidth, y);
	...
	// Boss name
	this.ctx.fillStyle = "white";
	this.ctx.font = "14px SpidermanGamePixelFont, Monospace, Helvetica";
	this.ctx.textAlign = "left";
	this.ctx.textBaseline = "top";
	this.ctx.fillText("BOSS: " + boss.name + " [" + boss.health + "/" + boss.maxHealth + "]", x + 5, y + 3);
}
...
SpidermanGame.prototype.drawRoofs = function() {
	...
	if (roofs.length < 3) {
		...
		// Remove gap - buildings are connected with bridges
		var x = lastRoof.x + lastRoof.fullWidth;
...
SpidermanGame.prototype.update = function() {
	...
	this.ctx.textAlign = "center";
	this.ctx.textBaseline = "top";
	this.ctx.fillText("LEVEL " + this.level + " | SCORE: " + this.score, this.canvas.width / 2, 5);
	...
	// Check for level completion (every 5 enemies defeated)
	if (this.enemiesDefeated > 0 && this.enemiesDefeated % 5 === 0 && !this.bossActive) {
		this.spawnBoss();
	}
...
SpiderMan.prototype.update = function() {
	...
	// Allow backward movement but don't let camera go below 0
	if (this.x < this.game.cameraX) {
		if (this.game.cameraX > 0) {
			// Move camera backward with player
			this.game.cameraX = Math.max(0, this.x);
		} else {
			// At start of level, don't go past left edge
			this.x = 0;
		}
	}

	// Don't let player go off right side of screen
	var maxX = this.game.cameraX + this.canvas.width - img.width * this.scale - 10;
	if (this.x > maxX) {
		this.x = maxX;
	}

	// Move camera forward when player is past center
	if (this.x - this.game.cameraX > 150 && this.velocityX > 0) {
...
function Roof(game, x, y) {
	...
	// Don't spawn enemy on first roof (x === 0)
	var shouldSpawnEnemy = this.x > 0 && Math.round(Math.random() * 100) >= 30;
	if (shouldSpawnEnemy) {
...
function Boss(game, opts) {
	...
	this.bossType = opts.bossType || "VENOM";
	this.level = opts.level || 1;
	this.name = this.bossType.replace(/_/g, " ");
	
	// Boss stats scale with level
	this.maxHealth = 20 + (this.level * 10);
	this.health = this.maxHealth;
	this.damage = 2 + this.level;
	this.shootSpeed = Math.max(30, 100 - (this.level * 10));
	
	this.x = opts.x || this.canvas.width - 50;
	this.y = opts.y || 0;
	this.scale = 0.7; // Bosses are bigger
	this.stateImg = this.game.resources[this.bossType] || this.game.resources.VENOM;
...
Boss.prototype.update = function() {
	var img = this.game.resources[this.bossType] || this.game.resources.VENOM;
	this.stateImg = img;
	...
	this.movementPattern += 0.05;
	this.x += Math.sin(this.movementPattern) * this.movementSpeed;
	
	this.ctx.save();
	this.ctx.scale(-1, 1);
	this.ctx.drawImage(img, (x + width) * -1, y, width, height);
	this.ctx.restore();
	...
	if (this.frame % this.shootSpeed === 0 && isInScreen) {
		this.shoot();
	}
...
Boss.prototype.remove = function() {
	// Bosses give more score and web
	this.game.score += this.maxHealth * 2;
	this.game.spiderman.web += this.maxHealth * 2;
	this.game.removeEnemy(this);
}

Boss.prototype.handleHitWithProjectile = function(projectile) {
	if (projectile.name == "WEB") {
		this.health -= projectile.damage;
		this.wasDamagedOnPreviousFrame = true;
	}
...
Enemy.prototype.update = function() {
	...
	var distanceToPlayer = spiderman.x - this.x;
	var chaseSpeed = 1.5 * this.game.physicsCoefficient();
	
	// Move toward player if they're close enough
	if (Math.abs(distanceToPlayer) > 50 && Math.abs(distanceToPlayer) < 400) {
		if (distanceToPlayer > 0) {
			this.x += chaseSpeed; // Move right toward player
		} else {
			this.x -= chaseSpeed; // Move left toward player
		}
	}
	...
	// Shoot more frequently (every 60 frames instead of 100)
	if (this.frame % 60 === 0 && isInScreen) {
		this.shoot();
	}
